...
Includes temporal filtering: Detections must be matched within a radius 
over consecutive scans before being published to filter out noise.
...
class TrackedObstacle:
    """Helper class to keep state of obstacles across multiple scan frames."""
...
class LidarObstacleNode(Node):
    ...
    def __init__(self):
        ...
        # Scan filtering
        self.declare_parameter('min_range',           0.15)   # ignore returns closer than this (m)
        self.declare_parameter('max_range',           3.0)    # ignore returns beyond this (m)
        self.declare_parameter('fov_fraction',        0.1)    # fraction of scan arc each side of fwd

        # Gap segmentation
        self.declare_parameter('gap_threshold',       0.15)   # >10cm between adjacent pts = new segment
        self.declare_parameter('min_segment_points',  10)     # ignore tiny noise segments
        self.declare_parameter('max_segment_points',  80)     # ignore large wall segments
        ...
        # Centerline filter
        self.declare_parameter('waypoints_csv', '/home/nvidia/ros2_ws/src/roboracer-final-race/pure_pursuit/path/waypoints_oncar.csv')
        self.declare_parameter('max_centerline_dist', 0.3)    # reject centroids further than this (m)

        # Temporal Tracking filter
        self.declare_parameter('temporal_match_radius', 0.5)  # Max dist to associate frame-to-frame (m)
        self.declare_parameter('min_publish_hits',      3)    # Scans required to publish
        self.declare_parameter('max_misses',            1)    # Allowed consecutive missed scans before dropping track
        ...
        self.temporal_match_radius = self.get_parameter('temporal_match_radius').value
        self.min_publish_hits      = self.get_parameter('min_publish_hits').value
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        if len(r) < self.min_seg_pts:
            self._update_tracks_and_publish([], msg.header)
            return
        ...
        dx   = np.diff(x_laser)
        dy   = np.diff(y_laser)
        gaps = np.sqrt(dx*dx + dy*dy)

        split_at = np.where(gaps > self.gap_threshold)[0] + 1
        ...
        for s, e in zip(starts, ends):
            ...
            current_centroids.append((cx_map, cy_map))

        # 6. Track temporal history and publish
        header          = Header()
        header.stamp    = msg.header.stamp
        header.frame_id = self.map_frame

        self._update_tracks_and_publish(current_centroids, header)

    # ────────────────────────────────────────────────────────────────────────
    def _update_tracks_and_publish(self, current_centroids, header):
        """Matches current detections to past tracks, updates history, and publishes."""
        unassigned_tracks = self.tracks.copy()
        new_tracks = []
        ...
        for cx, cy in current_centroids:
            ...
            # If a track is found within the radius threshold
            if best_track is not None and best_dist <= self.temporal_match_radius:
                # Update the track's position (simple replacement)
                best_track.x = cx
                best_track.y = cy
                best_track.hits += 1
                best_track.misses = 0
                ...
                # Spawn a new track
                new_tracks.append(TrackedObstacle(cx, cy))

        # Any track left unassigned wasn't seen in this frame
        for t in unassigned_tracks:
            t.misses += 1

        # Prune dead tracks and update state
        self.tracks = [t for t in self.tracks if t.misses <= self.max_misses]
        self.tracks.extend(new_tracks)

        # Filter strictly for publishing: >= 5 hits AND currently visible (misses == 0)
        publish_centroids = [
            (t.x, t.y) for t in self.tracks 
            if t.hits >= self.min_publish_hits and t.misses == 0
        ]

        if not publish_centroids:
            self._publish_empty(header.stamp)
            return

        self._publish_centroids(header, publish_centroids)
        self._publish_markers(header, publish_centroids)

        self.get_logger().debug(f'Tracking {len(self.tracks)} objects, Published {len(publish_centroids)} obstacle(s)')
    ...
    def _publish_markers(self, header: Header, centroids):
        ...
        lifetime_ns = int(2 * (1.0 / 15.0) * 1e9)
...
#         # Scan filtering
#         self.declare_parameter('min_range',           0.15)   # ignore returns closer than this (m)
#         self.declare_parameter('max_range',           2.0)   # ignore returns beyond this (m)
#         self.declare_parameter('fov_fraction',        0.1)    # fraction of scan arc each side of fwd
