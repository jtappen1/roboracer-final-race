...
// ─────────────────────────────────────────────
static constexpr double WHEELBASE      = 0.3; // 0.15;
static constexpr double L_MIN          = 0.7; // 0.5;
static constexpr double L_MAX          = 3.0;
static constexpr double V_MIN          = 2.5; // 3;
static constexpr double V_MAX          = 6.0; // 5;
...
public:
    PurePursuit() : Node("pure_pursuit_node") {
        ...
        // ── load waypoints ──────────────────────────────────────
        this->declare_parameter<std::string>("waypoints_path", "/home/nvidia/ros2_ws/src/roboracer-final-race/pure_pursuit/path/waypoints_oncar.csv");
...
private:
    ...
    void load_waypoints(const std::string& path) {
        ...
        while (std::getline(f, line)) {
            ...
            if (std::getline(ss, tok, ',')) wp.v = std::stof(tok);  // velocity is col 3
            if (std::getline(ss, tok, ',')) wp.l = std::stof(tok);  // lookahead is col 4
